...
class EncoderConvLSTM_stoch(nn.Module):
    def __init__(self, args, dataset_train, latent_dim=512, hidden_dim=1024, lstm_layers=1, attention=False, n_classes=26, backbone='resnet50', pretrain=False):
        super(EncoderConvLSTM_stoch, self).__init__()
        self.camera_s = {'mean': {'train': dataset_train.camera_s_mean}, 'std': {'train': dataset_train.camera_s_std}}
        self.camera_u = {'mean': {'train': dataset_train.camera_u_mean}, 'std': {'train': dataset_train.camera_u_std}}
        self.camera_v = {'mean': {'train': dataset_train.camera_v_mean}, 'std': {'train': dataset_train.camera_v_std}}
    ...
    def forward(self, x, muNsig=True):
        ...
        mu = self.output_mu(x)                          # [batch_size*seq_size, n_classes]
        logvar = self.output_var(x)                        # [batch_size*seq_size, n_classes]
        # Stochastic Inference
        x = self.reparameterize(mu, logvar)
        # Get hand joints
        x2d, x3d, camera_param, theta, beta = self.getHandJoints(x, batch_size, seq_size)
        if muNsig == True :
            return x2d, x3d, camera_param, theta, beta, mu, logvar
        else : return x2d, x3d, camera_param, theta, beta
    ...
    def getHandJoints(self, xs, b_size, seq_size):
        ...
        beta = xs[:, 16:]  # Beta       10
        scale = scale * torch.tensor([self.camera_s['std']['train']]).cuda() + torch.tensor([self.camera_s['mean']['train']]).cuda()
        trans = trans * torch.tensor([self.camera_u['std']['train'], self.camera_v['std']['train']]).unsqueeze(0).cuda() + torch.tensor([self.camera_u['mean']['train'], self.camera_v['mean']['train']]).unsqueeze(0).cuda()
