...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.feature_extractor(x)                   # [batch_size*seq_size, 2048, 1, 1]
        x = x.view(x.size(0), -1)                       # [batch_size*seq_size, 2048]
        x = self.final(x)                               # [batch_size*seq_size, 512]
        return x


class EncoderBase(nn.Module):
    def __init__(self, args, dataset_train, latent_dim=512, hidden_dim=1024, attention=False, n_classes=26, backbone='resnet50', pretrain=False):
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)                             # [batch_size*seq_size, 512]
